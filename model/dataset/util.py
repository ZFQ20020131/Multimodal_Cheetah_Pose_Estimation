...
def audio_loader(npz_path, n_channels, H=224, W=224):
    """
    npz_path: 每个音频对应的 npz 文件路径
    n_channels: 输出特征通道数
    H, W: 输出特征的空间尺寸（可调整）
    """
    data = np.load(npz_path)
    emb = data['embedding']  # shape: (dino_feature_dim,) or CLAP embedding dimension

    # If the embedding dimension is less than the number of channels, padding or repetition is required.
    if emb.shape[0] < n_channels:
        repeat_times = int(np.ceil(n_channels / emb.shape[0]))
        emb = np.tile(emb, repeat_times)[:n_channels]
    elif emb.shape[0] > n_channels:
        emb = emb[:n_channels]

    # Convert to float32
    emb = emb.astype(np.float32)

    # Extend the H and W dimensions
    # emb = emb[:, None, None]  # Cx1x1
    # emb = np.tile(emb, (1, H, W))  # CxHxW

    return emb  # np.ndarray, shape: (C,H,W)
