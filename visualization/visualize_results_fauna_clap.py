...
from model.utils.misc import setup_runtime
from model.render.mesh import make_mesh
import model.render.renderutils as ru
from model.geometry.skinning import estimate_bones, skinning, euler_angles_to_matrix
from model.render import util
from model.dataloaders import get_data_loaders
...
@hydra.main(config_path="../config", config_name="test_fauna")
def main(cfg):
    ...
    if "rotation" in render_modes or "canonicalization" in render_modes or "animation" in render_modes:
        ...
        cfg.dataset.batch_size = 1
        cfg.dataset.num_frames = 1
    input_image_dir = cfg.dataset.test_data_dir
    num_frames = 1
    ...
    image_paths = sorted(Path(input_image_dir).rglob("*_rgb.*"))
    save_basenames = [osp.splitext(osp.basename(p))[0].replace('_rgb', "") for p in image_paths]
    ...
    for i in tqdm(range(0, total_num, batch_size)):
        ...
        # 1. Load input images.
        images = [pil_loader(p) for p in image_paths[i:min(i + batch_size, total_num + 1)]]
        ...
        if cfg.finetune_texture:
            ...
            mask_path = str(image_paths[i]).replace("_rgb"+rgb_ext, "_mask.png")
            ...
            box_path = str(image_paths[i]).replace("_rgb"+rgb_ext, "_box.txt")
            ...
            cfg.dataset.load_dino_cluster = False
            cfg.dataset.data_type = "image"
            cfg.dataset.train_data_dir = osp.dirname(single_data_dir)
            dataloader, _, _ = get_data_loaders(cfg.dataset)

            for iteration in range(num_iters):
                for batch in dataloader:
                    batch = validate_all_to_device(batch, device)
                    
                    prior_shape, dino_pred, bank_embedding = model.netBase(
                        total_iter=total_iter, is_training=False, batch=batch, bank_enc=model.netInstance.netEncoder
                    )
                    shape, pose_raw, pose, mvp, w2c, campos, texture_pred, im_features, deform, all_arti_params, light, forward_aux = \
                        model.netInstance(batch[0], prior_shape, epoch, total_iter, is_training=False)
                    ...
                    image_pred = shaded[:, :3, :, :]
                    image_gt = batch[0].squeeze(1)  # [1, 3, 256, 256]
                    image_loss = 1 * (image_pred - image_gt).abs().mean()
                    optimizer.zero_grad()
                    image_loss.backward()
                    optimizer.step()
        ...
        with torch.no_grad():
            ...
            prior_shape, dino_pred, bank_embedding = model.netBase(total_iter=total_iter, is_training=False, 
                                                                   batch=[input_image], bank_enc=model.netInstance.netEncoder)
            shape, pose_raw, pose, mvp, w2c, campos, texture_pred, im_features, deform, all_arti_params, light, forward_aux = \
            ...
            if "animation" in render_modes:
                # 加载CLAP结果
                clap_result_file = osp.join(cfg.dataset.test_data_dir, "clap_results.json")
                clap_results = load_clap_results(image_paths, clap_result_file)
                print(f"Loaded CLAP results for {len(clap_results)} frames from {clap_result_file}")
                ...
                deformed_bones, kinematic_tree, _ = estimate_bones(
                    deformed_shape.v_pos[:, None, :, :], model.netInstance.cfg_articulation.num_body_bones,
                    n_legs=model.netInstance.cfg_articulation.num_legs,
                    n_leg_bones=model.netInstance.cfg_articulation.num_leg_bones,
                    body_bones_mode=model.netInstance.cfg_articulation.body_bones_mode, compute_kinematic_chain=True,
                ...
                # Defining the Foot Bone Index (determined according to the skeletal structure in 3DFauna)
                foot_bone_indices = [9, 10, 12, 13, 15, 16, 18, 19]
                ...
                for arti_id, arti_param in enumerate(animate_arti_params):
                    ...
                    # Constructing a stable window + smooth confidence level
                    f_seq = [pred['predicted_class']==0 for pred in clap_results]
                    c_seq = [float(pred['confidence']) for pred in clap_results]
                    D, c_smooth = build_contact_windows(f_seq, c_seq, beta=0.8, tau=0.55, L_min=4, G_max=3)
                    # Perform foot correction
                    arti_param = apply_audio_guided_adjustment(arti_params, D, c_smooth, foot_bones,
                                  alpha_y=0.3, alpha_z=0.2,
                                  theta_min=None, theta_max=None)

                    verts_articulated, aux = skinning(
                        deformed_shape.v_pos, deformed_bones, kinematic_tree, arti_param, output_posed_bones=True,
                        temperature=model.netInstance.cfg_articulation.skinning_temperature)
...
def validate_all_to_device(batch, device):
    return tuple(validate_tensor_to_device(x, device) for x in batch)


def load_clap_results(image_paths, clap_result_file):
    """Load CLAP classification results"""
    # Suppose there is a file containing the classification results for all audio segments.
    if not os.path.exists(clap_result_file):
        print(f"CLAP result file {clap_result_file} not found, proceeding without CLAP constraints")
        return None
    
    with open(clap_result_file, 'r') as f:
        clap_results = json.load(f)
    return clap_results


def adjust_articulation_based_on_clap(arti_params, clap_result, foot_bone_indices):
    """Adjust articulation parameters based on CLAP foot contact results"""
    # Copy original joint angles
    adjusted_arti_params = arti_params.clone()
    
    # If CLAP detects foot contact, adjust the foot joint angle.
    if clap_result and clap_result['predicted_class'] == 0:  # Foot Contact
        # Increase the bending angle of the foot joints to bring the foot closer to the ground.
        confidence = clap_result['confidence']
        for bone_idx in foot_bone_indices:
            # Adjust the angle according to the confidence level; the higher the confidence level, the greater the adjustment.
            adjusted_arti_params[:, :, bone_idx, 0] -= 0.5 * confidence  # Adjust X-axis rotation
            adjusted_arti_params[:, :, bone_idx, 2] -= 0.5 * confidence  # Adjust the Z-axis rotation to make the foot flatter.
    
    return adjusted_arti_params


def build_contact_windows(f_seq, c_seq, beta=0.8, tau=0.55, L_min=4, G_max=3):
    T = len(c_seq)
    # EMA smoothing
    c_smooth = [0.0]*T
    prev = c_seq[0]
    for t in range(T):
        prev = (1 - beta) * c_seq[t] + beta * prev
        c_smooth[t] = prev

    # Gating
    gate = [1 if c_smooth[t] > tau else 0 for t in range(T)]

    # Extraction of consecutive segments
    segs, t = [], 0
    while t < T:
        if gate[t] == 1:
            s = t
            while t + 1 < T and gate[t + 1] == 1:
                t += 1
            e = t
            segs.append([s, e])
        t += 1

    # Minimum Continuous Filtering
    segs = [seg for seg in segs if (seg[1] - seg[0] + 1) >= L_min]
    ...
    for seg in segs:
        if not merged:
            ...
            prev = merged[-1]
            if seg[0] - prev[1] - 1 <= G_max:
                prev[1] = seg[1]
            else:
                merged.append(seg)

    D = [(s, e) for s, e in merged]
    return D, c_smooth

def apply_audio_guided_adjustment(arti_params, D, c_smooth, foot_bones,
                                  alpha_y=0.3, alpha_z=0.2,
                                  theta_min=None, theta_max=None):
    """
    arti_params: [T, ..., num_bones, 3]  # y,z 分别位于 [:, :, :, 1] 和 [:, :, :, 2]
    foot_bones: list[int]  # 足部骨骼索引
    """
    T = len(c_smooth)
    adjusted = arti_params.copy() 

    # Pre-generated mask (O(1) query)
    mask = [0]*T
    for (s, e) in D:
    ...
    for t in range(T):
        if mask[t] == 1:
            dy = alpha_y * c_smooth[t]
            dz = -alpha_z * c_smooth[t]
            for b in foot_bones:
                adjusted[t, ..., b, 1] += dy  # y-axis
                adjusted[t, ..., b, 2] += dz  # z-axis
                # Limit (if upper and lower limits are provided)
                if theta_min is not None and theta_max is not None:
                    adjusted[t, ..., b, 1] = min(max(adjusted[t, ..., b, 1], theta_min[b][1]), theta_max[b][1])
                    adjusted[t, ..., b, 2] = min(max(adjusted[t, ..., b, 2], theta_min[b][2]), theta_max[b][2])
